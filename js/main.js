...
function applySettings() {
  ...
  if (s.whatsapp) {
    ...
    if (fab) fab.href = `https://wa.me/${num}?text=Hi%20Gadgets360.np%2C%20I%27d%20like%20to%20enquire%20about%20a%20product.`;
  }
}
...
function renderProducts(filter) {
  ...
  allFeatured = products.filter(p => p.featured === 'yes' && p.status === 'active');

  const catMap = { smartphones: 'smartphones', laptops: 'laptops', audio: 'audio', gaming: 'gaming' };
  const filtered = activeFilter === 'all'
    ? allFeatured
    : allFeatured.filter(p => p.category === (catMap[activeFilter] || activeFilter));

  if (filtered.length === 0) {
    grid.innerHTML = `<div style="grid-column:1/-1;text-align:center;padding:48px;color:var(--text-3)">No products in this category yet.</div>`;
    return;
  }
        ...
        <div class="product-body">
          ...
          <div class="product-footer">
            ...
            <div class="product-actions">
              <a href="https://wa.me/${waNum}?text=${orderMsg}" target="_blank" rel="noopener" class="btn-wa-sm" title="Order via WhatsApp">
  ...
  form.addEventListener('submit', (e) => {
    ...
    form.parentElement.innerHTML = `
      <div class="form-success">
        ...
        <p>We've received your message and will reach out shortly via WhatsApp or Instagram.</p>
        <br/>
...
const animateEls = document.querySelectorAll(
  '.category-card, .why-card, .testimonial-card, .deal-card, .contact-channel, .about-card'
);
...
}, { threshold: 0.4 }).observe(sections[0] || document.body);
